<script lang="ts">
    ...
    function connect(name: string) {
        console.log("Calling dispatch with name", name)
    ...
    function closeModal(
        ...
        if (e.target === backdrop || e.target === container) {
            console.log("Calling dispatch wth close!")
            dispatch("close");
        }
    }

    function handleKeyup(e: KeyboardEvent & {currentTarget: EventTarget & Window }) {
        if (e.key == "Escape") {
            dispatch("close");
        }
    }
...
<svelte:window on:keyup={e => handleKeyup} />

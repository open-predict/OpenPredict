<script lang="ts">
    ...
    $: ({ publicKey, sendTransaction, signTransaction, confirmTransaction } =
        $web3Workspace);
...
<div class="modal_card gap-4 p-4">
    ...
    >
        <p class="whitespace-nowrap">
            ...
            {/if}
            <span
                class={` ${direction ? "text-emerald-500" : "text-rose-500"}`}
            >
    ...
    <p class="text-sm text-gray-500 whitespace-pre-wrap">
        {#if buying}
            ...
            )} & ${$web3Store.sol ? $web3Store.sol?.toFixed(2) : 0} SOL`}
        {:else}

<script lang="ts">
		...
		if (magic && (!_publicKey || (_publicKey && $web3Store?.magicWallet))) {
			...
			if (loggedIntoMagic) {
				...
				if (!magicMetadata.publicAddress)
					throw new Error(
						"Logged into magic without a solana address"
					);
	...
	async function refreshUsdcAddress() {
		...
		if (publicKey && connection) {
			try {
				...
				const account = await web3spl.getAccount(
					connection,
					accountPubKey
				);
		...
		try {
			...
			async function signProcess(publicKey: PublicKey) {
				...
				if ($walletStore.connected && $walletStore.signMessage) {
					...
					if (magic) {
						signedMessage = await magic.solana.signMessage(
							challenge
						);
					} else {
	...
	function supportVersionedTransactions() {
		const wallet = $walletStore.connected &&
			$walletStore.wallet &&
	...
	async function handleTransaction(
		...
		try {
			...
			var swapAddressLookup = undefined;
			if (microUsdc !== undefined) {
				onStatus(TxStatus.SWAPPING);
					try {
					console.log("tx needs usdc, old instructions", instructions);
					console.log("adding swap instructions...");
					const [swapInstructions, addressLookupTables] =
						await swapCoinsInstructions(
							connection,
							publicKey,
							web3spl.NATIVE_MINT,
							usdcMintAddr,
							microUsdc,
							!supportVersionedTransactions(),
						)
					swapAddressLookup = addressLookupTables;
					instructions = [...swapInstructions, ...instructions];
					console.log(
						"new instructions",
						instructions.map((v) =>
							v.keys.map((v2) => v2.pubkey.toString())
						)
					);
				} catch (e) {
			...
			const {
				...
				signedTx,
				error: signTxError,
			} = await signTransaction(instructions, swapAddressLookup);
			if (signTxError || !signTxId || !signedTx) {
				onError(
					signTxError ?? new Error("Unable to sign transaction.")
				);
				return;
			}
			...
			const { id: sendTxId, error: sendTxError } = await sendTransaction(
				signTxId,
				signedTx
			);
			if (sendTxError || !sendTxId) {
				onError(sendTxError ?? new Error("Unable to send transaction"));
				return;
			}

			onStatus(TxStatus.CONFIRMING);

			const { slot, txError, error } = await confirmTransaction(sendTxId);
			if (error || !!txError || slot === undefined) {
				onError(
					txError
						? new Error(txError.toString())
						: error ?? new Error("Unable to confirm transaction")
				);
				return;
			}
